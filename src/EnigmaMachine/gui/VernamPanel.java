...
public class VernamPanel extends JPanel{
    ...
    public VernamPanel(ViewManager view){
        viewManager = view;
    ...
    private void makeObjects(){
        ...
        cipherName = new JLabel("Vernam Cipher");
        ...
        cipherIntro = new JLabel("<html>Assume each letter in the alphabet is numbered from 0-25.<br> "
                + "The Vernam cipher converts the letters in the plaintext and the key to bits according to their lexicographical ordering and then performs an XOR with the key.<br>"
                + "The key and plaintext have to be the same length for the Vernam cipher to work.<html>");
    ...
    */
    private void decrypt(){
        if(checkKey()){
            if(checkInput()){
                if(checkPair()){
                    String answer = EnigmaMachine.decryptVigenere(input.getText(), key.getText());
                    output.setText(answer);
                }
                else{
                    Dialogs.error(viewManager, "The key and text you have entered have different lengths", "Invalid key, text pair");
                }
            }
            else{
    ...
    */
    private void encrypt(){
        if(checkKey()){
            if(checkInput()){
                if(checkPair()){
                    String answer = EnigmaMachine.encryptVigenere(input.getText(), key.getText());
                    output.setText(answer);
                }
                else{
                    Dialogs.error(viewManager, "The key and text you have entered have different lengths", "Invalid key, text pair");
    ...
    */
    ...
    private boolean checkPair(){
        if(input.getText().length() == key.getText().length()){
            return true;
        }
        return false;
    }
